...
func (s *Server) recv(c *net.UDPConn) {
	...
	for {
		...
		if err := s.parsePacket(buf[:n], from); err != nil {
			// removing spam for local dev - AG  17.03.22
...
func (s *Server) probe() {
	...
	for i := 0; i < 3; i++ {
		if err := s.SendMulticast(q); err != nil {
			log.Errorf("[ERR] mdns: failed to send probe: %s", err.Error())
		}
	...
	for i := 0; i < 3; i++ {
		if err := s.SendMulticast(resp); err != nil {
			log.Errorf("[ERR] mdns: failed to send announcement: %s", err.Error())
		}
		select {
		case <-timer.C:
			timeout *= 2

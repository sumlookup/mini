...
func TestToLongServiceNameForMDNS(t *testing.T) {
	s := Service{
		Name:    "test5",
		Version: "1.0.5",
		Nodes: []*Node{
			{
				Id:      "suuuuuuuuuper-tooooooo-looooooooooong-2563f520fd5ecae1d5a356148f9b7be2",
				Address: "10.0.0.5:10005",
	...
	assert.Error(t, err, "register new service should return error")
	assert.EqualErrorf(t, err, "service instance name is too long", "the error msg is not valid")
}
...
func TestMDNS(t *testing.T) {
	...
	testData := []*Service{
		...
		},
		{
			Name:    "test4",
			Version: "1.0.4",
			Error:   "service instance name is too long",
			Nodes: []*Node{
				{
					Id:      "suuuuuuuuuper-looooooooooong-se-2563f520fd5ecae1d5a356148f9b7be2",
	...
	for _, service := range testData {
		...
		if err := r.Register(service); err != nil {
			if service.Error != "" {
				if err.Error() != service.Error {
					t.Fatalf("Expected error %s got %s", service.Error, err.Error())
				}
				continue
			}
		}

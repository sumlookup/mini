...
import (
	...
	"github.com/sumlookup/mini/util/mdns"
)
...
func (m *mdnsRegistry) Register(service *Service, opts ...RegisterOption) error {
	...
	for _, node := range service.Nodes {
		...
		logger.Debugf("[mdns] registry create new %s@%s on %s:%v", service.Name, service.Version, net.ParseIP(host).String(), port)
